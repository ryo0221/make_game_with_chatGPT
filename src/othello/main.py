...
from othello.ai import RandomAI, MinimaxAI, QLearningAgent
...
class OthelloGUI:
    def __init__(self, mode="PVP"):
        ...
        self.ai_white = None
        if mode == "PVAI":
            self.ai_white = QLearningAgent(player=WHITE, epsilon=0.0)
            with open("q_table.pkl", "rb") as f:
                self.ai_white.q_table = pickle.load(f)
        elif mode == "AIvAI":
            self.ai_black = QLearningAgent(player=BLACK, epsilon=0.0)
            with open("q_table.pkl", "rb") as f:
                self.ai_black.q_table = pickle.load(f)
            self.ai_white = RandomAI(player=WHITE)
